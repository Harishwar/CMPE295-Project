 ...
 */
public class MagnetometerTableOperations extends DatabaseConnector {
    protected static final String MAGNETOMETER_TABLE = "magnetometer_table";

    ContentValues contentValues = new ContentValues();

    public MagnetometerTableOperations(Context context)
    {
        super(context);
    }
    ...
    public void insertValue(double xVal, double yVal, double zVal, String timeStamp)
    {
        try {
            ...
            db.insert(MAGNETOMETER_TABLE, "id", contentValues);
            db.close();
        }
        catch (Exception ex)
        {
    ...
    {
        try {
            String sql = "select count(*), TOTAL(" + columnName + ") from '" + MAGNETOMETER_TABLE + "' where timestamp>'" + datetimeStart + "' and timestamp<='" + datetimeEnd + "'";
    ...
    {
        try{
            String sql = "delete from "+MAGNETOMETER_TABLE+" where timestamp>"+datetimeStart+" and timestamp<"+datetimeEnd;
            return true;
        }
        catch (Exception ex)
        {
